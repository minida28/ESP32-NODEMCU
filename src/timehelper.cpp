...
void Timeloop()
{
    ...
    static uint32_t secOld = 0;
    uint32_t currSec = (uint32_t)tv.tv_sec;
    if (secOld != currSec)
    {
        secOld = currSec; // store new value

        // showTime();

        // DEBUGLOG("%u,%u | %u,%u\r\n", (uint32_t)tv.tv_sec, (uint32_t)tv.tv_usec, (uint32_t)tp.tv_sec, (uint32_t)tp.tv_nsec);
    }
    ...
    if (!timeIsSynced)
    {
        ...
        if ((uint32_t)now > 1574007545)
        {
            DEBUGLOG("Time is synced!\r\n")
